...
На головному `promedia.report` у блоці «Новини» автоматично відображаються
три найсвіжіші матеріали, позначені редакцією як важливі.
...
Якщо підключений серверний `OPENAI_API_KEY`, під час створення або збереження
новини адмінка автоматично заповнює порожні англомовні поля та теги, не
перезаписуючи ручні правки автора.

Опублікований контент доступний як машинозчитуваний архів за адресою
`/api/export/published-news.json`. GitHub Actions щодня зберігає його копію
в `archive/published-news.json`; чернетки, користувачі, паролі та секрети до
цього архіву не потрапляють.
...
Щоб увімкнути автопереклад новин англійською та автотеги, додайте серверний
ключ OpenAI:
...
Ключ зберігається тільки як секрет Cloudflare Worker і не передається в
браузер. За замовчуванням використовується модель `gpt-5-mini`; за потреби
можна задати іншу модель через змінну середовища `OPENAI_MODEL`.
...
src/
  ...
  lib/
    ...
    articleAssist.js — автопереклад новин, SEO-опис EN і автотеги
